 ...
 */
 ...
 /*
function contarParaArriba(numeroInicial, numeroLimite) {
  for (let numero = numeroInicial; numero <= numeroLimite; numero++) {
    console.log(numero);
  }
}
*/

// contarParaArriba(1, 10);
...
function imprimirNumerosImpares(comienzo, fin) {
  for (comienzo; comienzo <= fin; comienzo += 2) {
    console.log(comienzo);
  }
}

imprimirNumerosImpares(primerNumero, ultimoNumero);
*/
...
/* function tablaDeTalNumero(talNumero, numeroFinal) {
  let i = 0;
  while (i <= numeroFinal) {
    console.log(talNumero * i);
    i++;
  }
}
let numeroTabla = 7;
let numeroFinalTabla = 9;

tablaDeTalNumero(numeroTabla, numeroFinalTabla);
*/

/*
Desafío de programación #4: Imprimí todas las tablas de multiplicación del 1 al 9, pero que la formula
funcione con cualquier numero al 9 (tabla de multiplicación del 1 al 15 que vaya del 1 al 9)
...
/*
function tablaDeTalNumero(talNumero, numeroFinal, hastaTalNumero, matrizTabla1al9) {
  while (talNumero <= numeroFinal) {
    let matrizVaCambiando = [];
    // console.log(`Imprimiendo tabla del ${talNumero}`);
    for (let i = 0; i <= hastaTalNumero; i++) {
      let k;
      k = talNumero * i;
      // console.log(k);
      matrizVaCambiando.push(k);
    }
    matrizTabla1al9.push([`Tabla del ${talNumero}: ${matrizVaCambiando}`]);
    talNumero++;
  }
  /* console.log(matrizTabla1al9); Lo que hizo la función es guardar las tablas de los números requeridos
  del 1 al 9
  // return matrizTabla1al9;
}
*/
/* 
let numeroInicialTabla = 1;
let numeroFinalTabla = 10;
let hastaTalNumeroTabla = 9;
let matriz = [];
tablaDeTalNumero(numeroInicialTabla, numeroFinalTabla, hastaTalNumeroTabla, matriz);
console.log(matriz);
*/
...
/*
function sumarVector(numeroInicial, numeroFinal){
  let sumaVector = [];
  let resultado = 0;
  for(let i = numeroInicial; i <= numeroFinal; i++){ // Forma un vector con los numeros indicados al llamar la funcion
    sumaVector.push(numeroInicial);
    numeroInicial++;
  }
  console.log(`El vector quedó conformado asi: ${sumaVector}`);
  for(i = 0; i < sumaVector.length; i++){ // Suma los numeros del vector en una variable "resultado"
    resultado += sumaVector[i];
  }
  console.log(`El resultado de la suma de estos números es: ${resultado}`);
}

let primerNumero = 1;
let ultimoNumero = 10;

sumarVector(primerNumero, ultimoNumero);
*/
...
function calcularFactorial(numeroFactorial){
  let resultadoFactorial = numeroFactorial;
...
function calcularFarenheit(numeroCelsius){
  let numeroFarenheit;
  numeroFarenheit = numeroCelsius * 9 / 5 + 32
  console.log(`Tu número (${numeroCelsius}°C) es igual a ${numeroFarenheit}°F`);
}

let numeroCelsius = prompt("Ingrese un número en grados Celsius para obtener el equivalente en grados Farenheit");

calcularFarenheit(numeroCelsius);
*/
...
function encontrarNumeroMaximoVector(vectorDeNumeros){
  let numeroMaximoVector = vectorDeNumeros[0];
  console.log(numeroMaximoVector); // Este es el primer número del vector
  for(let i = 0; i < vectorDeNumeros.length; i++){
    if(numeroMaximoVector < vectorDeNumeros[i]){
      numeroMaximoVector = vectorDeNumeros[i];
    }
  }
  console.log(`El número máximo de este vector es: ${numeroMaximoVector}`)
}

let numerosFavoritos = [-104, 8, 10, 5, 1, 101, 32, -50, 69, -69];

encontrarNumeroMaximoVector(numerosFavoritos);
/*
